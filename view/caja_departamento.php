...
<div class="container-fluid px-1" id="cajadep">
    <div class="row justify-content-center">

        <div class="col-md-6 text-center mb-5 mt-3">
            <h2 class="men" style="width: 70%;">Caja Departamento</h2>
        </div>
    </div>
    <br>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="row in paginaCollection" style="text-align:center">
                            <td>{{row.id_caja_departamento}}</td>
                            ...
                            <td>{{row.nombre_departamento}}</span></td>
                            <!-- <td>{{row.nombre_caja}}</span></td> -->
                            <td>{{row.historico_descripcion}}</span></td>
                            <td>{{row.esactivo}}</td>
                            <td><a :href="'../view/historial_departamento.php?id_caja_departamento=' + row.id_caja_departamento+'&id_caja='+row.id_caja+'&id_sucursal='+row.id_sucursal+'&id_empresa='+row.id_empresa"><i class="fa-solid fa-clock-rotate-left fa-lg" style="color: #6500a8;"></i> Historial</a></td>
                            <td>
                                <button style="background:none;border:none" @click="abrirModal(row.id_caja_departamento,'Editar', row);"><i class="fa-solid fa-square-pen fa-lg" style="color: #3976c6;"></i></button>
                                <button style="background:none;border:none" @click="eliminarCajaD(row.id_caja_departamento);"><i class="fa-solid fa-trash fa-lg" style="color: #f50f0f;"></i></button>
    ...
    <b-modal v-model="modal" scrollable no-close-on-backdrop>
            ...
            <div>
                <b-form>
                    <b-form-group class="mb-10 mt-10" label="Nombre de la Caja:">
                        <b-form-input type="text" v-model="nombre_caja_departamento" placeholder="Ingresa el nombre" require></b-form-input>
                    </b-form-group>
                    <b-form-group class="mb-10 mt-10" label="Empresa:">
                        <b-form-select v-model="id_empresa" disabled>
                            ...
                            <b-form-select-option v-for="row in empresaColleccion" v-bind:key="row.id_empresa" :value="row.id_empresa" disabled>{{row.nombre_empresa}}</b-form-select-option>
                        </b-form-select>
                    </b-form-group>
                    ...
                    <b-form-group class="mb-10 mt-10" label="Selecciona el Departamento:" v-if="title == 'Agregar'">
                    ...
                    <b-form-group class="mb-10 mt-10" label="Selecciona el Departamento:" v-if="title == 'Editar'">
        ...
        <div slot="modal-footer" class="w-100">
            <b-button v-if="title == 'Agregar'" variant="success" class="float-right ml-2" @click="insertarCajaD();"><i class="fa-solid fa-floppy-disk"></i></b-button>
            <b-button v-else variant="primary" class="float-right ml-2" @click="editarCajaD();"><i class="fa-regular fa-pen-to-square"></i></b-button>
...
<script type="text/javascript" src="../js/caja/caja_departamento.js"></script>
